...
import com.example.cognicare.presenters.Point;
import com.example.cognicare.presenters.PointType;
import com.example.cognicare.presenters.PresenterCompletableObserver;
import com.example.cognicare.presenters.PresenterObserver;
...
public class TetrisGameModel implements GameModel {
    ...
    private static final int PLAYING_AREA_WITH = 10;
    ...
    public void init() {
        ...
        mPlayingPoints = new Point[PLAYING_AREA_HEIGHT][PLAYING_AREA_WITH];
        for(int i = 0; i < PLAYING_AREA_HEIGHT; i++) {
            System.arraycopy(mPoints[i], 0, mPlayingPoints[i], 0, PLAYING_AREA_WITH);
        }
        ...
        for(int i = 0; i < UPCOMING_AREA_SIZE; i++) {
            for(int j = 0; j < UPCOMING_AREA_SIZE; j++) {
                mUpcomingPoints[i][j] = mPoints[1 + i][PLAYING_AREA_WITH + 1 + j];
            }
        }
        for (int i = 0; i < PLAYING_AREA_HEIGHT; i++) {
            mPoints[i][PLAYING_AREA_WITH].type = PointType.VERTICAL_LINE;
        }
        newGame();
    }
    ...
    public void newGame() {
        mScore = 0;
        for(int i = 0; i < PLAYING_AREA_HEIGHT; i++) {
            for (int j = 0; j < PLAYING_AREA_WITH; j++) {
                mPoints[i][j].type = PointType.EMPTY;
            }
        }
    ...
    private synchronized void next() {
        ...
        if(isNextMerged()) {
            ...
            while(y >= 0) {
                ...
                for(int i = 0; i < PLAYING_AREA_WITH; i++) {
    ...
    private void updatePlayingPoint(Point point) {
        if (point.x >= 0 && point.x < PLAYING_AREA_WITH && point.y >= 0 && point.y < PLAYING_AREA_HEIGHT) {
    ...
    private Point getPlayingPoint(int x, int y) {
        if (x >= 0 && y >= 0 && x < PLAYING_AREA_WITH && y < PLAYING_AREA_HEIGHT) {
    ...
    public void turn(GameTurn turn) {
        ...
        switch (turn) {
            ...
            case RIGHT:
                ...
                canTurn = true;
                for(Point fallingPoint : mFallingPoints) {
                    if(fallingPoint.y >= 0 && (fallingPoint.x == PLAYING_AREA_WITH - 1 || getPlayingPoint(fallingPoint.x + 1, fallingPoint.y).isStablePoint())) {
                        canTurn = false;
                        break;
                    }
                }
    ...
    private boolean rotatePoints(int x, int y, int size) {
        if(x + size - 1 < 0 || x + size - 1 >= PLAYING_AREA_WITH) {
            return false;
        }
    ...
    @Override
    public void setScoreUpdateListener(PresenterObserver<Integer> onScoreUpdateListener) {
        mScoreUpdatedObserver = onScoreUpdateListener;
    }
